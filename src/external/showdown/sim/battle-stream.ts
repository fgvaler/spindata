...
export function getPlayerStreams(stream: BattleStream) {
	const streams = {
		omniscient: new ObjectReadWriteStream({
			write(data: string) {
		...
		p1: new ObjectReadWriteStream({
			write(data: string) {
				void stream.write(data.replace(/(^|\n)/g, `$1>p1 `));
			},
		}),
		p2: new ObjectReadWriteStream({
			write(data: string) {
				void stream.write(data.replace(/(^|\n)/g, `$1>p2 `));
			},
		}),
		p3: new ObjectReadWriteStream({
			write(data: string) {
				void stream.write(data.replace(/(^|\n)/g, `$1>p3 `));
			},
		}),
		p4: new ObjectReadWriteStream({
			write(data: string) {
				void stream.write(data.replace(/(^|\n)/g, `$1>p4 `));
			},
		}),
...
export abstract class BattlePlayer {
	...
	constructor(playerStream: ObjectReadWriteStream<string>, debug = false) {
...
export class BattleTextStream extends ReadWriteStream {
